...
joined = all_aggregates_mean.set_index(join_index).join(
    all_aggregates_std.set_index(join_index), rsuffix="_std"
)
...
# %%
all(
    joined.reset_index().set_index(join_index)[optimization_targets].sort_index()
    == all_totals.groupby(join_index).apply(np.mean)[optimization_targets].sort_index()
)
...
# %%
t = (
    experiment_data.groupby(level=minima.index.names)
    .apply(lambda x: df_argmin(x, target))
    .reset_index(level=[3, 4, 5])
    .drop(columns=group_keys)
    .reset_index(level=["platform", "exp"])
)

t[["timestamp", "timestamp_std"]].apply(
    lambda x: x
)  # + "+-" + t["timestamp_std"].to_string()
(
    t["timestamp_std"].map(lambda x: "{:.3f}".format(x))
    + " +- "
    + t["timestamp"].map(lambda x: "{:.3f}".format(x))
)
...
long_platform = {"dgx9": "V100", "k80-2": "K80", "t4-1": "T4"}
...
for target in optimization_targets:
    ...
    for platform, platform_data in joined.groupby(level="platform"):
        ...
        for experiment, experiment_data in platform_data.groupby(level="exp"):
            print("")
            table = (
                experiment_data.groupby(level=experiment_data.index.names)
                .apply(lambda x: df_argmin(x, target))
                .reset_index(level=[3, 4, 5])
                .drop(columns=group_keys)
                .reset_index(level=["platform", "exp"])
            ...
            for column in optimization_targets:
                table[column] = (
                    "$"
                    + table[column].map(lambda x: "{:.3f}".format(x))
                    + " \pm "
                    + table[column + "_std"].map(lambda x: "{:.3f}".format(x))
                    + "$"
                )
...
(
    (
        1
        / all_totals[
            (all_totals.platform == "dgx9") & (all_totals.benchmark == "nbody")
        ]
        .groupby(join_index)
        .mean()
        .reset_index()
...
long_targets = {"energy": "energy", "timestamp": "time", "edp": "EDP", "ed2p": "ED^2P"}
...
long_platform = {"dgx9": "V100", "k80-2": "K80"}

for target in optimization_targets:
    projected_columns = ["run", target] + [
        x for x in optimization_targets if x != target
    ]
...
minima.groupby(level=minima.index.names).apply(
    lambda x: df_argmin(x, "energy")
).reset_index(level=[3, 4, 5])
...
# %%
all_aggregates[
    (all_aggregates.platform == "dgx9")
    & (all_aggregates.exp == "power-limit")
    & (all_aggregates.benchmark == "bert")
]
...
def select_platform_and_experiment(df):
    return df[(df.platform == "dgx9") & (df.exp == "clock-limit")].set_index(
        "benchmark"
    )


# %%
a = select_platform_and_experiment(min_t)[["timestamp", "run"]].rename(
    columns={"timestamp": "$t_{min}$ $[s]$", "run": "$P_{t_{min}}$"}
)

# %%
b = select_platform_and_experiment(min_e)[["energy", "run"]].rename(
    columns={"energy": "$E_{min}$ $\left[kJ\right]$", "run": "$P_{E_{min}}$"}
)

# %%
c = select_platform_and_experiment(min_edp)[["edp", "run"]].rename(
    columns={"edp": "$EDP_{min}$ $\left[kJs\right]$", "run": "$P_{EDP_{min}}$"}
)
...
print(
    ...
    .to_latex(
        float_format="%.3f",
        escape=False,
        caption="This table lists the optimal (minimizing) clock limit "
...
# %%
def select_platform_and_experiment(df):
    return df[(df.platform == "k80-2") & (df.exp == "clock-limit")].set_index(
        "benchmark"
    )
...
# %%
df[df.platform == "dgx9"].reset_index().set_index("benchmark").rename(
    columns={"run": "best time", "timestamp": "time"}
)
