...
def calculate_derived_metrics(df, p_baseline, p_max, p_max_nominal, util):
    ...
    df["p_util"] = df.groupby("benchmark")["power"].transform(
        lambda power: get_activity_from_power(
            power=power, p_max=p_max, p_max_nominal=p_max_nominal
        )
    )
    df["util_gpu_norm"] = df["util-gpu"] / 100

    df["time_reference"] = df.groupby("benchmark")["timestamp"].transform(
        lambda time: time.loc[f"{p_max_nominal}W"].mean()
    )
    df["delay"] = (
        df["timestamp"] / df["time_reference"]
    )  # all_agg.groupby("benchmark")["timestamp"].transform(lambda time: time/time.loc["300W"].mean())
    ...
    df["relative_power"] = np.clip(
        df["enforced_power_limit"] / df[util], a_max=p_max, a_min=None
    )
...
class ClockModel:
    ...
    def _prepare(self, data):
        ...
        data["relative_power"] = np.clip(
            data["power_limit"] / data["p_util"], a_max=self.p_max, a_min=None
        )

    def fit(self, power_limit, p_util, clock_gpu):
        data = pd.DataFrame(
            {"power_limit": power_limit, "clock_gpu": clock_gpu, "p_util": p_util}
        )
        self._prepare(
            data
        )  # power sm.ols(formula="relative_clock_gpu ~ relative_power", data=data).fit()
    ...
    def predict(self, power_limit, p_util):
        data = pd.DataFrame({"power_limit": power_limit, "p_util": p_util})
        self._prepare(data)
        return np.clip(
            self._model.predict(data), a_max=self.f_max, a_min=None
        )  # * f_max
    ...
    def plot(self, training_data, util, x="enforced_power_limit"):
        ...
        training_data["clock_gpu_hat"] = self.predict(
            training_data["enforced_power_limit"], training_data[util]
        )
    ...
    def plot_raw(self, training_data, util):
        ...
        training_data = training_data.rename(
            columns={"enforced_power_limit": "power_limit"}
        )
        self._prepare(training_data)
        training_data["clock_gpu_hat"] = self.predict(
            training_data["power_limit"], training_data[util]
        )

        sns.scatterplot(
            x="relative_power", y="clock_gpu", hue="benchmark", data=training_data
        )
...
class PowerModel:
    ...
    def fit(self, p_util, clock_gpu, power):
        data = pd.DataFrame({"p_util": p_util, "clock_gpu": clock_gpu, "power": power})
        self._prepare(data)
    ...
    def plot(
        self, training_data, util, p_max_nominal, x="clock_gpu", hue: str = "benchmark"
    ):
        training_data = training_data.copy()
        all_predictions = []
        for b in training_data["benchmark"].unique():
            ...
            x_pred = np.linspace(
                util_selection["clock_gpu"].min(),
                util_selection["clock_gpu"].max(),
                100,
            )
            y_pred = self.predict(
                util_selection.loc[f"{p_max_nominal}W"][util].mean(), x_pred
            )
            df = self._df4pred(
                util_selection.loc[f"{p_max_nominal}W"][util].mean(), x_pred
            )
        ...
        sns.lineplot(
            x=x, y="power_hat", data=all_predictions, hue=hue
        )  # , style=list("xxxxxxxxxxxxxxx"), markers={"x":"x"}) ,hue="benchmark"
...
class DelayModel:
    ...
    def fit(self, clock_gpu, delay):
        data = pd.DataFrame({"clock_gpu": clock_gpu, "delay": delay})
        self._prepare(data)
        self._model = sm.ols(formula="delay ~ d1", data=data).fit()

    def predict(self, clock_gpu):
        data = pd.DataFrame({"clock_gpu": clock_gpu})
        self._prepare(data)
...
class EnergyModel:
    ...
    def plot(self, training_data, x="enforced_power_limit", hue: str = "benchmark"):
        ...
        training_data["energy"] = (
            training_data["energy"]
            + (self.power_model.baseline * training_data["timestamp"]) / 1000
        )

        training_data["energy_hat"] = self.predict(
            p_util=training_data["p_util"],
        ...
        sns.lineplot(
            x=x, y="energy_hat", hue="benchmark", data=training_data
        )  # , style=list("xxxxxxxxxxxxxxx"), markers={"x":"x"}) ,hue="benchmark"
...
class EnergyDelayProductModel:
    ...
    def predict(self, power_limit, p_util, time_reference):
        ...
        data["clock_gpu_hat"] = self.clock_model.predict(power_limit, p_util)
        data["delay_hat"] = self.delay_model.predict(data["clock_gpu_hat"])
        data["energy"] = self.energy_model.predict(
            p_util=data["p_util"],
    ...
    def plot(self, training_data, x="enforced_power_limit", hue: str = "benchmark"):
        ...
        training_data["energy"] = (
            training_data["energy"]
            + (self.energy_model.power_model.baseline * training_data["timestamp"])
            / 1000
        )
        training_data["edp"] = training_data["energy"] * training_data["timestamp"]

        training_data["edp_hat"] = self.predict(
            p_util=training_data["p_util"],
            power_limit=training_data["enforced_power_limit"],
            time_reference=training_data["time_reference"],
        )
        ...
        sns.lineplot(
            x=x, y="edp_hat", hue="benchmark", data=training_data
        )  # , style=list("xxxxxxxxxxxxxxx"), markers={"x":"x"}) ,hue="benchmark"
        plt.xlabel("Power Limit [W]")
        plt.ylabel("EDP [kJs]")
