...
def load_power_cap_data(data_root, network, epoch_count):
    ...
    for cap in power_caps:
        for run in runs:
            ...
            tmp = {
                "relative_time": np.arange(len(cum_energy)),
                "power_cap": cap,
                "energy": energy,
...
def plot_mean_metric(df, metric, net, scale):
    ...
    plt.plot(
        [x for x in energy.keys()],
        np.array([np.array(x).mean() for x in energy.values()]) / scale,
        "x",
    )
...
# ecg_data = load_power_cap_data("../data/data-1.3", "ecg", epoch_count = 20)
...
ax = sns.lineplot(
    x="relative_time", y="cum_energy", hue="power_cap", data=mnist_data_big
)
